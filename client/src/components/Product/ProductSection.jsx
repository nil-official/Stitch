...
const ProductSection = ({ productId, scrollToReviews }) => {
    ...
    return (
        product.id && (
            <div className={`lg:grid lg:grid-cols-2 lg:gap-x-8 lg:items-start ${productLoading ? "opacity-20" : "opacity-100"}`}>
                {/* Left column */}
                ...
                <div className="mt-10 px-4 sm:px-0 sm:mt-16 lg:mt-0">
                    ...
                    <div className="mt-10 flex sm:flex-col1 space-x-4">
                        ...
                        <button
                            ...
                            disabled={cartLoading || isOutOfStock || !size}
                            className={`flex-1 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white transition-all transform ${cartLoading || isOutOfStock || !size ? 'bg-[#cdcdcd] cursor-default' : 'bg-black hover:bg-gray-800'}`}
                        >
                            {cartLoading ? (
